...
static LSM6DS3 *globalImu = NULL;

ISR(TIMER2_OVF_vect) {
  sei();
  globalImu->integrateGyroZ();
}
...
//
//****************************************************************************//
LSM6DS3Core::LSM6DS3Core(uint8_t inputArg) : I2CAddress(0x6B) {
...
//
//****************************************************************************//
LSM6DS3::LSM6DS3(uint8_t inputArg) : LSM6DS3Core(inputArg) {
  ...
  settings.gyroSampleRate =
      104; // Hz.  Can be: 13, 26, 52, 104, 208, 416, 833, 1666
  ...
  settings.accelSampleRate = 104; // Hz.  Can be: 13, 26, 52, 104, 208, 416,
...
//****************************************************************************//
float LSM6DS3::mblocCall(int input) {
  switch (input) {
  case 1:
    return (readFloatAccelX());
    break;
  case 2:
    return (readFloatAccelY());
    break;
  case 3:
    return (readFloatAccelZ());
    break;
  case 4:
    return (readFloatGyroX());
    break;

  case 5:
    return (readFloatGyroY());
    break;
  case 6:
    return (readFloatGyroZ());
    break;
  case 7:
    return (readTempC());
    break;
  case 8:
    ...
    break;
  default:
    return (0); // Default to full 2000DPS range
  }
}
//****************************************************************************//
//
...
float LSM6DS3::readGyroZ() {
  ...
  cli();
  float delta = (float)gyroZDelta * 4.375 * (gyroRangeDivisor) / 1000;
  delta -= gyroZSamples * gyroZOffset;
  gyroZSamples = 0;
  gyroZDelta = 0;
  gyroZ += delta * (1./settings.gyroSampleRate);
  sei();

  return gyroZ;
}

void LSM6DS3::integrateGyroZ() {
  gyroZSamples++;
  gyroZDelta += readRawGyroZ();
}

void LSM6DS3::resetGyroZ() { gyroZ = 0; }

void LSM6DS3::tareGyroZ() {
  TIMSK2 = 0;
  double tmp = 0;
  delay(10);
  for (int k = 0; k < 100; k++) {
